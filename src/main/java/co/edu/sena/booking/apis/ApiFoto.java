...
import co.edu.sena.booking.jpa.controllers.FotoJpaController;
...
public class ApiFoto extends BasicApi implements IApi{
    ...
    private FotoJpaController fotoController = null;
     ...
     private ApiFoto() {
        fotoController = new FotoJpaController(Utils.getEM());
        gson = JsonTransformer.singleton().getGson();
        init();
    }
     
    public static ApiFoto singleton() {
        if (instance == null) {
            instance = new ApiFoto();
        }
        return instance;
    }
    ...
    public Object create(Request rq, Response rs) {
        ...
        try {
            String body = rq.body();
            Foto nEntity = gson.fromJson(body, Foto.class);
            fotoController.create(nEntity);
    ...
    public Object update(Request rq, Response rs) {
        ...
        try {
            ...
            String body = rq.body();
            Foto nEntity = gson.fromJson(body, Foto.class);
            Foto oEntity = fotoController.findFoto(id);
            if (oEntity != null) {
                oEntity.setFotRuta(nEntity.getFotRuta());
                oEntity.setFotLabel(nEntity.getFotLabel());
                oEntity.setFotDescripcion(nEntity.getFotDescripcion());
                oEntity.setFkAlojamiento(nEntity.getFkAlojamiento());
                oEntity.setFkLugar(nEntity.getFkLugar());
                fotoController.edit(oEntity);
                rs.status(201);
                retorno.put("status", 201);
    ...
    public Object getAll(Request rq, Response rs) {
        Hashtable<String, Object> retorno = new Hashtable<>();
        List<Foto> entities = fotoController.findFotoEntities();
        if (entities.size() > 0) {
            retorno.put("status", 200);
    ...
    public Object getById(Request rq, Response rs) {
        ...
        try {
            int id = Integer.parseInt(rq.params("id"));
            Foto entity = fotoController.findFoto(id);
            if (entity != null) {
                retorno.put("status", 200);
